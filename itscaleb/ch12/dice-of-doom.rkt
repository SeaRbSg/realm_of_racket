...
(define (pass w)
  (define m (find-move (game-moves (dice-world-gametree w)) '()))
  (cond [(not m) w]
  ...
  (define (attacks board)
    (for*/list ([src board]
		...
		#:when (attackable? board player src dst))
	       ...
	       (define gt-attack
		 (game newboard player (delay (cons (passes newboard) (attacks newboard)))))
	       ;(define moremoves (cons (passes newboard) (attacks newboard)))
	       (move (list from dst) gt-attack)));(game newboard player moremoves))))
  (define (passes board)
...
(define (rate-moves tree depth)
  (for/list ([move (game-moves tree)])
	    (list move (rate-position (move-gametree move) (- depth 1)))))
...
(define (the-ai-plays tree)
  ...
  (define new-tree (move-gametree the-move))
